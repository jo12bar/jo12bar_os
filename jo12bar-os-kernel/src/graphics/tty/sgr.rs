...
/// An ANSI SGR (Select Graphic Rendition).
///
/// This represents possible changes to the styling of text that is displayed in
/// a TTY.
///
...
/// All [SGR][AnsiSGR] parsing-related errors.
...
impl SGRParseState {
    fn next(&mut self, c: char) {
        match self {
            ...
            SGRParseState::Err(e) => panic!("called next on \"Err\" state: {e}"),
            SGRParseState::Done(_) => log::error!("called next on \"Done\" state"),
    ...
    fn parse_color(c: char, state: &mut ColorParseState) {
        match state {
            ...
            ColorParseState::Err(e) => panic!("called next on \"Err\" state: {e}"),
            ColorParseState::Done(_) => log::error!("called next on \"Done\" state"),
        }
    }
    ...
    fn expect_char(&mut self, current: char, expected: char, next: SGRParseState) {
        if current != expected {
            *self = SGRParseState::Err(SGRParseError::ExpectedChar {
                expected,
                got: current,
            });
            return;
...
impl AnsiSGR {
    /// Parse ANSI SGR (Select Graphic Rendition) from char iterator.
    ///
    /// If `no_esc` is set, it's assumed that the ESC char in the SGR was
    /// already read and the next char is expected to be a `[`.
    /// Otherwise the first two chars are expected to be `\0x1b[`.
    ///
    /// See: <https://chrisyeh96.github.io/2020/03/28/terminal-colors.html>
